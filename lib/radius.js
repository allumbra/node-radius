...
Radius.lookup_request_code = function (codeId) {
  return code_map[codeId]
}
...
Radius.lookup_attribute_code = function (attribute_code, vendor) {
  try {
    var vendor = vendor || NO_VENDOR;
    var attr_info = attributes_map[vendor] && attributes_map[vendor][attribute_code];
    return attr_info[ATTR_NAME];
  } catch (e) {
    console.error('key not found - vendor:' + vendor + ' attribute_code: ' + attribute_code)
    return null;
  }
}

Radius.lookup_attribute_info = function (attribute_code, vendor) {
  try {
    var vendor = vendor || NO_VENDOR;
    var attr_info = attributes_map[vendor] && attributes_map[vendor][attribute_code];
    return attr_info;
...
Radius.attribute_info_map = (attributeCodes) =>
  _.reduce(attributeCodes,
    function (acc, code) {
      var inf = Radius.lookup_attribute_info(parseInt(code), NO_VENDOR);  // TODO, handle VSA
      if (_.size(inf) > 1) {
        acc[code] = {
          id: inf[ATTR_ID],
          name: inf[ATTR_NAME],
          type: inf[ATTR_TYPE],
          enum: inf[ATTR_ENUM],
          rev_enum: inf[ATTR_REVERSE_ENUM],
          modifiers: inf[ATTR_MODIFIERS]
        }
      } else {
        console.log("Unknown code: " + code, inf);
      }
      return acc;
    },
    // function(acc, code){ return _.assign(acc, {code: Radius.lookup_attribute_info(parseInt(code, NO_VENDOR))})}, 
    {});  // TODO: handle vsa's


Radius.lookup_code = function (code) {
...
Radius.decode_attributes = function (data, attr_hash, vendor, raw_attrs) {
  ...
  while (data.length > 0) {
    type = data.readUInt8(0);  // type will be 26 for VSA's
    ...
    if (vendor > 100) {
      attr_hash["vendors"] = attr_hash["vendors"] || {};
      attr_hash["vendors"][vendor.toString()] = vendor.toString();
    }
    // dictionary lookup
...
///////////////////////////////////////////////////////////////////////////////
//  utility functions   TODO - reorganize this
...
Radius.stringDecoder = (value) => value.toString("utf8");
Radius.ipDecoder = (value) => {
    var octets = [];
    for (var i = 0; i < value.length; i++) {
        octets.push(value[i]);
    }
...
Radius.dateDecoder = (value) => new Date(value.readUInt32BE(0) * 1000);
// also used for time
Radius.integerDecoder = (value, has_tag) => {
    if (has_tag) {
        var buf = new Buffer([0, 0, 0, 0]);
        value.copy(buf, 1);
        value = buf;
...
Radius.decodeValue = (val, attInf) => {
    attInf = attInf || defaultAttrInfo;
    switch (attInf.type) {
        case "string":
        case "text":
            // assumes utf8 encoding for strings
            result = Radius.stringDecoder(val);
            break;
        case "ipaddr":
            result = Radius.ipDecoder(val);
            break;
        case "date":
            result = Radius.dateDecoder(val);
            break;
        case "time":
        case "integer":
...
Radius.base64MapToBuffer = (map) => _.reduce(map,
    (acc, value, key) => {
        acc[key] = Buffer.from(value, 'base64');
        return acc;
    },
    {});
    Radius.decodeVsa = (data) => {
    ...
    var attrInfo = Radius.lookup_attribute_info(type, vendorId);
    
    var vsaName = (attrInfo && attrInfo[ATTR_NAME]) || ("DNF:" + vendorId + ":" + type);
    var result = {};
    console.log(vsaName);
    if(attrInfo){
      result[vsaName] = Radius.decodeValue(value, {type: attrInfo[ATTR_TYPE], enum: attrInfo[ATTR_ENUM], modifiers: attrInfo[ATTR_MODIFIERS]});
    }else{
      console.log("VSA info not found: (vendorId, type, attrInfo)", vendorId, type, attrInfo);
    }
    return result;
}
Radius.decodeBufferMap = (map) => {
    var attInfoMap = Radius.attribute_info_map(_.keys(map));
    var vsas = {};
    var decBuffMap =  _.reduce(map, (acc, val, code) => {
        ...
        if (code == 26) {  // process VSA
          try{
            ...
            result = Radius.decodeValue(val,attInf);
        }
...
Radius.decodeNpsRadius = (obj) => {
    var result = {};
    result.code = Radius.lookup_request_code(obj.requestCode);
    var bufferMap = Radius.base64MapToBuffer(obj.request);    
    result.attributes = Radius.decodeBufferMap(bufferMap);
    return result;
}
